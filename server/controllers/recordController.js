...
// @TODO search
  async getAll(req, res, next) {
    try {
      ...
      const records = await Record.findAndCountAll({limit, offset, include: [{model: Request, as: 'request'}]});
  ...
  async getOne(req, res, next) {
    try {
      const {id} = req.params;
      const record = await Record.findOne({where: {id}, include: [{model: Request, as: 'request'}]});
